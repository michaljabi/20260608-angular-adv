...
async function seed(): Promise<void> {
  ...
  if (force) {
    ...
    if (existing.length > 0) {
      console.log('Already seeded — skipping (use --force to reseed).');
      return;
    }
  }

  const auctionRows = seedAuctions.map((a) => ({ ...a, uid: nanoid(8) }));
  db.insert(auctions).values(auctionRows).run();
  console.log(`Seeded ${auctionRows.length} auctions.`);
